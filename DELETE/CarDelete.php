...
<body>
  <div class="form">
    ...
    <?
     ...
     $id = intval(str_replace('http://taxi/DELETE/CarDelete.php?id=','',$url));

     $queryCar = $mysqli->query("SELECT * FROM `Car` WHERE ID_CAR=$id");
     $taxman;
     while ($dataCar = mysqli_fetch_array($queryCar)) {
       $taxman =  intval($dataCar['ID_TAXMAN']);
      }

    echo $taxman; 

    $queryTaxman = $mysqli->query("SELECT * FROM `Taxman` WHERE ID_TAXMAN=$taxman");
    $fullname_p;
    while ($dataTaxman = mysqli_fetch_array($queryTaxman)) {
      $fullname_p = $dataTaxman['Fullname'];
    } 
    ...
    <h2>Вы уверены что хотите удалить автомобиль таксиста - <?= $fullname_p ?></h2>
